...
from peft.tuners.lora.dora import DoraConv2dLayer, DoraEmbeddingLayer, DoraLinearLayer
...
from optimized_module.index_select import EffIndexSelectFn
...
class LoraMoeLayer(BaseTunerLayer):
    ...
    def update_layer(
        self, adapter_name, r, num_gates, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_dora: bool = False
    ):
        ...
        self.lora_A[adapter_name] = nn.ModuleList([nn.Linear(self.in_features, r, bias=False) for _ in range(num_gates)])
        self.lora_B[adapter_name] = nn.ModuleList([nn.Linear(r, self.out_features, bias=False) for _ in range(num_gates)])
        if use_rslora:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in self.lora_A.keys():
                    continue
                lora_As = self.lora_A[active_adapter]
                lora_Bs = self.lora_B[active_adapter]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = x.to(lora_As[0].weight.dtype)
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    for i in range(len(gates_idx)):
                        if gates_idx[i].numel() == 0:
                            continue
                        x_sub = EffIndexSelectFn(x_dropout, dim=gate_idx_dim, index=gates_idx[i])
                        im = lora_As[i](x_sub)
                        lora_delta = lora_Bs[i](im) * scaling
                        result = torch.index_add(result, dim=gate_idx_dim, index=gates_idx[i], source=lora_delta)
